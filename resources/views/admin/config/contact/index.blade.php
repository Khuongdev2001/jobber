...
@section('js')
    ...
    <script>
       ...
       var table= $('.table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.config.contact.get") }}',
        columns: [
    ...
    $(document).on("click",".table .btn-delete",function (){
        ...
        Swal.fire({
            title: 'Xóa Liên Hệ',
            text: "Việc xóa sẽ không thể khôi phục được",
            icon: 'warning',
    ...
    actionAll("{{route('admin.config.contact.delete')}}","{{route('admin.config.contact')}}",{title:"Thông báo",text:"xóa liên hệ đã chọn"});
